...
u32 MPSICardParty(u32 idx, u32 numParties, u32 numElements, std::vector<block> &set, u32 numThreads)
{
    oc::CuckooParam params = oc::CuckooIndex<>::selectParams(numElements, ssp, 0, 3);
    u32 numBins = params.numBins();

    MShuffleParty64 shuffleParty64(idx, numParties, numBins);       
    shuffleParty64.getShareCorrelation("psic");   
    ...
    std::vector<std::vector<u64>> vvecOPPRF(numParties - 1);// for P_0
    ...
    if(idx == 0){
        ...
        for (u32 i = 0; i < numBins; ++i)
        {
            ...
            if (bin.isEmpty() == false)
            {
                ...
                //share[i] = block(halfK, 0);  //x||0                                                             
            }                              
        }        

        for (u32 i = 1; i < numParties; ++i)
        {             
            std::string opprfPath = "./offline/vole_" + std::to_string(numParties) + "_" + std::to_string(numElements) + "_P" + std::to_string(idx + 1) + std::to_string(i + 1) + "PSI"; 
            opprfThrds[i - 1] = std::thread([&, i, opprfPath]() {                
                opprfRecvPSI64(numParties, numElements, cuckooSeed, cuckooHashTable, chl[i - 1], vvecOPPRF[i - 1], opprfPath, numThreads);
            });                               
        }
    	...
    	coproto::sync_wait(shuffleParty64.runXOR(chl, shareSum));
        ...
        std::string opprfPath = "./offline/vole_" + std::to_string(numParties) + "_" + std::to_string(numElements) + "_P" + std::to_string(idx + 1) + std::to_string(1) + "PSI"; 
        opprfSendPSI64(numParties, numElements, cuckooSeed, simHashTable, chl[0], vecOPPRF, opprfPath, numThreads);

        std::string postfixPath2 = std::to_string(numParties) + "_" + std::to_string(numElements) + "_P" + std::to_string(idx + 1) + std::to_string(numParties) + "PSI64"; 
        beaverTripleMulOnline64(idx, numParties, vecOPPRF, chl, vecBTMul, postfixPath2);

        coproto::sync_wait(shuffleParty64.runXOR(chl, vecBTMul));
