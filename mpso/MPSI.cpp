...
std::vector<u64> MPSIParty(u32 idx, u32 numParties, u32 numElements, std::vector<block> &set, u32 numThreads)
{
    ...
    if(idx == 0){
        ...
        // check MAC
        std::vector<u64> setUnion;
        for (size_t i = 0; i < numBins; i++){
            if (shareSum[i] == 0){
                setUnion.emplace_back(cuckooHashTable[i].mData[1]);                
            }
        }
