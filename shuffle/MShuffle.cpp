...
void MShuffleParty::getShareCorrelation(std::string fileName){
    std::string inFileName = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(mIdx + 1);
...
void MShuffleParty64::getShareCorrelation(std::string fileName){
    std::string inFileName = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(mIdx + 1);
    ...
    // read a for party 2 to party k
    if (mIdx != 0){
        inFile.read((char*)ma.data(), ma.size() * sizeof(u64));
    }

    // read aprime for party 1 to party k-1
    if (mIdx != mNumParties - 1){
        inFile.read((char*)maprime.data(), maprime.size() * sizeof(u64));
    }

    // read b for party 1 to party k-1
    if (mIdx != mNumParties - 1){
        inFile.read((char*)mb.data(), mb.size() * sizeof(u64));
    }

    // read delta for party k
    if (mIdx == mNumParties - 1){
        inFile.read((char*)mdelta.data(), mdelta.size() * sizeof(u64));
    }
}


Proto MShuffleParty64::runADD(std::vector<Socket> &chl, std::vector<u64> &data){
    // if (chl.size() != mNumParties - 1){
    //     std::cout << "number of channels is not correct" << std::endl;
    //     return Proto();
    // }
    MC_BEGIN(
        task<>, &chl, &data, this, i = u32{}, j = u32{}, 
        z = std::vector<std::vector<u64>>{}, zprime = std::vector<u64>{});
...
Proto MShuffleParty64::runXOR(std::vector<Socket> &chl, std::vector<u64> &data){
    // if (chl.size() != mNumParties - 1){
    //     std::cout << "number of channels is not correct" << std::endl;
    //     return Proto();
    // }
