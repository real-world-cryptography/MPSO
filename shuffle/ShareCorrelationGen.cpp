...
bool ShareCorrelation::exist(std::string fileName){
    ...
    for (u32 i = 1; i <= mNumParties; i++){
        inFileName[i - 1] = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(i);
    }
...
void ShareCorrelation::writeToFile(std::string fileName){
    ...
    for (size_t i = 1; i <= mNumParties; i++){
        outFileName[i - 1] = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(i);
    }
...
bool ShareCorrelationAdd::exist(std::string fileName){
    std::vector<std::string> inFileName(mNumParties);
    for (u32 i = 1; i <= mNumParties; i++){
        inFileName[i - 1] = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(i);
    }
...
void ShareCorrelationAdd::generate(std::string fileName){
    mPi.resize(mNumParties);
    ma.resize(mNumParties - 1);// without P1
    maprime.resize(mNumParties - 1);// without Pm
    mb.resize(mNumParties - 1);// without Pm
    mdelta.resize(mNumElements);// for Pm

    std::vector<std::string> inFileName(mNumParties);
    std::vector<std::ifstream> inFile(mNumParties);
    // read random permutation from u64
    for (size_t i = 0; i < mNumParties; ++i){
        inFileName[i] = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(i+1);// for u64
        inFile[i].open(inFileName[i], std::ios::binary | std::ios::in);

        if (!inFile[i].is_open()){
            std::cout << "opening file failed !" << inFileName[i] << "\n";
            return;
        }
        mPi[i].resize(mNumElements);
        inFile[i].read((char*)mPi[i].data(), mPi[i].size() * sizeof(u32));
    }
    ...
    for (size_t i = 0; i < mNumParties - 1; ++i){
        ...
        maprime[i].resize(mNumElements);
        mb[i].resize(mNumElements);
        prng.get<u64>(ma[i].data(), ma[i].size());
        prng.get<u64>(maprime[i].data(), maprime[i].size());
        prng.get<u64>(mb[i].data(), mb[i].size());
    }

    // compute delta
    mdelta.assign(mNumElements, 0);
    for (size_t i = 0; i < mNumParties - 1; ++i){
        for (size_t j = 0; j < mNumElements; ++j){
            mdelta[j] += ma[i][j];
        }
    ...
    for (size_t i = 0; i < mNumParties - 1; ++i){
        permute64(mPi[i], mdelta);
        for (size_t j = 0; j < mNumElements; ++j){
            mdelta[j] += maprime[i][j];
        }
    }
    permute64(mPi[mNumParties - 1], mdelta);
    for (size_t i = 0; i < mNumParties - 1; ++i){
        for (size_t j = 0; j < mNumElements; ++j){
            mdelta[j] -= mb[i][j];
        }
...
void ShareCorrelationAdd::writeToFile(std::string fileName){
    std::vector<std::string> outFileName(mNumParties);
    for (size_t i = 1; i <= mNumParties; i++){
        outFileName[i - 1] = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(i);
    }
    ...
    // write a for party 2 to party k
    for (size_t i = 0; i < mNumParties - 1; ++i){
        outFile[i + 1].write((char*)ma[i].data(), ma[i].size() * sizeof(u64));
    }

    // write aprime for party 1 to party k-1
    for (size_t i = 0; i < mNumParties - 1; ++i){
        outFile[i].write((char*)maprime[i].data(), maprime[i].size() * sizeof(u64));
    }

    // write b for party 1 to party k-1
    for (size_t i = 0; i < mNumParties - 1; ++i){
        outFile[i].write((char*)mb[i].data(), mb[i].size() * sizeof(u64));
    }

    // write delta for party k
    outFile[mNumParties - 1].write((char*)mdelta.data(), mdelta.size() * sizeof(u64));
...
void ShareCorrelationAdd::release(){
    std::vector<std::vector<u32>>().swap(mPi);
    std::vector<std::vector<u64>>().swap(ma);
    std::vector<std::vector<u64>>().swap(maprime);
    std::vector<std::vector<u64>>().swap(mb);
    std::vector<u64>().swap(mdelta);
}



bool ShareCorrelationXor::exist(std::string fileName){
    std::vector<std::string> inFileName(mNumParties);
    for (u32 i = 1; i <= mNumParties; i++){
        inFileName[i - 1] = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(i);
    }
...
// to do: realize from share translation
// when numParties and numElements grow, memory is not enough
void ShareCorrelationXor::generate(){
    mPi.resize(mNumParties);
    ma.resize(mNumParties - 1);
    maprime.resize(mNumParties - 1);
    mb.resize(mNumParties - 1);
...
void ShareCorrelationXor::writeToFile(std::string fileName){
    std::vector<std::string> outFileName(mNumParties);
    for (size_t i = 1; i <= mNumParties; i++){
        outFileName[i - 1] = "./offline/sc_" + fileName + "_" + std::to_string(mNumParties) + "_" + std::to_string(mNumElements) + "_P" + std::to_string(i);
    }
...
void ShareCorrelationXor::release(){
    std::vector<std::vector<u32>>().swap(mPi);
    std::vector<std::vector<u64>>().swap(ma);
    std::vector<std::vector<u64>>().swap(maprime);
    std::vector<std::vector<u64>>().swap(mb);
